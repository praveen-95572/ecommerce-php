...
if(isset($_GET['id']) && $_GET['id']!=''){
  ...
  $res=mysqli_query($conn,"select * from product where id='$id' $condition");
...
if(isset($_POST['submit'])){
  ...
  $sub_categories_id=get_safe_val($conn,$_POST['sub_categories_id']);
  ...
  $best_seller=get_safe_val($conn,$_POST['best_seller']);
  
  $res=mysqli_query($conn,"select * from product where name='$name' $condition");
  $check=mysqli_num_rows($res);
  if($check>0){
    if(isset($_GET['id']) && $_GET['id']!=''){
  ...
  if($msg==''){
    if(isset($_GET['id']) && $_GET['id']!=''){
      ...
      $task=mysqli_query($conn,$update_sql);
    }else{
      $image=rand(111111111,999999999).'_'.$_FILES['image']['name'];
      move_uploaded_file($_FILES['image']['tmp_name'],PRODUCT_IMAGE_SERVER_PATH.$image);
      $sql="insert into product(categories_id,name,mrp,price,qty,short_desc,description,meta_title,meta_desc,meta_keyword,status,img1,best_seller,sub_categories_id,added_by) values('$categories_id','$name','$mrp','$price','$qty','$short_desc','$description','$meta_title','$meta_desc','$meta_keyword',1,'$image','$best_seller','$sub_categories_id','".$_SESSION['ADMIN_ID']."')";
      $task=mysqli_query($conn,$sql);
    }
    if($task)
      echo "<script>alert('Task Executed');</script>";
